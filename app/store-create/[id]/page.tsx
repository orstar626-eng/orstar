...
import { ArrowRight, Save, Eye, EyeOff, QrCode } from 'lucide-react';
...
export default function StoreCreatePage() {
  ...
  return (
    <div className="min-h-screen bg-background" dir="rtl">
      ...
      <div className="sticky top-0 z-50 border-b border-border/60 bg-background/80 backdrop-blur-xl">
        <div className="flex items-center justify-between px-4 h-14 max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setShowQR(true)} disabled={!shortId} className="ios-press w-9 h-9 rounded-xl bg-secondary border border-border/60 flex items-center justify-center text-muted-foreground hover:bg-muted disabled:opacity-40 transition-all">
      ...
      <QRCodeModal isOpen={showQR} onClose={() => setShowQR(false)} url={shortId ? `${window.location.origin}/store/${shortId}` : ""} profileName={store?.name || "المتجر"} />
    </div>
  );
}
