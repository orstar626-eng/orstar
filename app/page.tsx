...
export default function HomePage() {
  ...
  const [shortenerAlias, setShortenerAlias] = useState('');
  ...
  const handleShorten = async () => {
    setShortenerError('');
    setShortenerResult('');
    const trimmed = shortenerUrl.trim();
    if (!trimmed) { setShortenerError('أدخل الرابط أولاً'); return; }
    if (!/^https?:\/\//.test(trimmed)) { setShortenerError('يجب أن يبدأ الرابط بـ http:// أو https://'); return; }
    setShortenerLoading(true);
    try {
      const params = new URLSearchParams({ format: 'json', url: trimmed });
      if (shortenerAlias.trim()) params.append('shorturl', shortenerAlias.trim());
      const res = await fetch(`https://is.gd/create.php?${params.toString()}`);
      const json = await res.json();
      if (json.shorturl) {
        setShortenerResult(json.shorturl);
      } else {
        setShortenerError(json.errormessage || 'حدث خطأ، حاول مرة أخرى');
      }
    } catch {
      setShortenerError('تعذّر الاتصال بالخدمة، تحقق من اتصالك');
    } finally {
      setShortenerLoading(false);
    }
  };

  const handleShortenerCopy = () => {
    if (!shortenerResult) return;
    navigator.clipboard.writeText(shortenerResult);
    setShortenerCopied(true);
    toast.success('تم نسخ الرابط المختصر');
                  ...
                  {profiles.map((p, index) => {
                    ...
                    return (
                      <div key={p.id} className="animate-ios-list-enter rounded-2xl border border-border/60 bg-card overflow-hidden hover:border-border hover:shadow-sm transition-all" style={{ animationDelay: `${index * 0.06}s` }}>
                        <div className="h-16 w-full relative" style={{ background: profile.theme?.backgroundType === 'gradient' ? `linear-gradient(135deg, ${profile.theme.gradientFrom}, ${profile.theme.gradientTo})` : profile.theme?.backgroundType === 'color' ? profile.theme.backgroundColor : 'linear-gradient(135deg, #0f172a, #1e1b4b)' }}>
                          <div className="absolute inset-0 bg-gradient-to-r from-black/10 to-transparent" />
                          {profile.avatar && <div className="absolute -bottom-5 right-4"><img src={profile.avatar} alt={profile.name} className="w-10 h-10 rounded-full ring-2 ring-card object-cover shadow-md" /></div>}
                          <div className="absolute top-2.5 left-3">
                            <span className="px-2 py-0.5 rounded-full bg-black/30 backdrop-blur-sm text-white text-[10px] font-medium font-mono">/p/{p.short_id}</span>
                          </div>
                        </div>
                    ...
                    <div className="h-14 w-full relative flex items-center px-4" style={{ background: store.theme?.backgroundColor }}>
                      ...
                      <div className="relative z-10 flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-white/60">/store/{s.short_id} · {store.products?.length || 0} منتج</p>
                        </div>
                      </div>
                    </div>
                    <div className="px-4 pb-4 pt-3">
            ...
            <div className="rounded-2xl border border-border/60 bg-card overflow-hidden">
              <div className="px-5 pt-5 pb-4 border-b border-border/40">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">تقصير الروابط</p>
                </div>
                <p className="text-[11px] text-muted-foreground/60 mt-1">اختصر أي رابط باستخدام خدمة is.gd</p>
              </div>
              <div className="p-5 space-y-3">
                <div>
                  <label className="text-xs font-medium text-muted-foreground mb-1.5 block">الرابط الأصلي</label>
                  <input
                    type="url"
                    value={shortenerUrl}
                    onChange={e => { setShortenerUrl(e.target.value); setShortenerResult(''); setShortenerError(''); }}
                    onKeyDown={e => e.key === 'Enter' && handleShorten()}
                    placeholder="https://example.com/very-long-url"
                    className="w-full px-3.5 py-2.5 rounded-xl border border-border/60 bg-background text-sm placeholder:text-muted-foreground/40 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary/50 transition-all font-mono text-xs"
                    dir="ltr"
                  />
                </div>
                <div>
                  <label className="text-xs font-medium text-muted-foreground mb-1.5 block">اسم مخصص للرابط <span className="text-muted-foreground/40">(اختياري)</span></label>
                  <div className="flex items-center gap-0 rounded-xl border border-border/60 bg-background overflow-hidden focus-within:ring-2 focus-within:ring-primary/30 focus-within:border-primary/50 transition-all">
                    <span className="px-3 text-xs text-muted-foreground/50 font-mono border-l border-border/40 py-2.5 bg-secondary/50 select-none shrink-0">is.gd/</span>
                    <input
                      type="text"
                      value={shortenerAlias}
                      onChange={e => { setShortenerAlias(e.target.value.replace(/[^a-zA-Z0-9_-]/g, '')); setShortenerResult(''); setShortenerError(''); }}
                      onKeyDown={e => e.key === 'Enter' && handleShorten()}
                      placeholder="my-link"
                      className="flex-1 px-3 py-2.5 bg-transparent text-sm placeholder:text-muted-foreground/40 focus:outline-none font-mono text-xs"
                      dir="ltr"
                      maxLength={30}
                    />
                  </div>
                  <p className="text-[10px] text-muted-foreground/40 mt-1">أحرف إنجليزية وأرقام و - و _ فقط</p>
                </div>
                <button
                  onClick={handleShorten}
                  disabled={shortenerLoading || !shortenerUrl.trim()}
                  className="ios-press w-full py-2.5 rounded-xl bg-foreground text-background text-sm font-bold hover:opacity-90 disabled:opacity-50 transition-all flex items-center justify-center gap-2"
                >
                  {shortenerLoading ? (
                  ...
                  <span>{shortenerLoading ? 'جاري التقصير...' : 'قصّر الرابط'}</span>
                </button>

                {shortenerError && (
                  <div className="flex items-center gap-2 px-3.5 py-2.5 rounded-xl bg-destructive/10 border border-destructive/20 text-destructive text-xs font-medium">
                ...
                {shortenerResult && (
                  <div className="flex items-center gap-2 px-3.5 py-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                    <a href={shortenerResult} target="_blank" rel="noopener noreferrer" className="flex-1 text-emerald-600 dark:text-emerald-400 text-xs font-mono font-bold hover:underline truncate">{shortenerResult}</a>
                    <button onClick={handleShortenerCopy} className="ios-press shrink-0 w-8 h-8 rounded-lg bg-emerald-500/20 flex items-center justify-center text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/30 transition-all">
                      {shortenerCopied ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
                    </button>
                  </div>
                )}
              </div>
