                  ...
                  {profiles.map((p, index) => {
                    ...
                    return (
                      <div key={p.id} className="animate-ios-list-enter rounded-2xl border border-border/60 bg-card overflow-hidden hover:border-border hover:shadow-sm transition-all" style={{ animationDelay: `${index * 0.06}s` }}>
                        <div className="h-14 w-full relative flex items-center px-4" style={{ background: profile.theme?.gradientFrom || profile.theme?.backgroundColor || '#0f172a' }}>
                          <div className="absolute inset-0" style={{ background: `linear-gradient(135deg, ${profile.theme?.accentColor || '#818cf8'}25, transparent)` }} />
                          <div className="relative z-10 flex items-center gap-3">
                            {profile.avatar && !profile.hideAvatar
                              ? <img src={profile.avatar} alt={profile.name} className="w-9 h-9 rounded-xl object-cover shadow-md ring-2 ring-white/10" />
                              : <div className="w-9 h-9 rounded-xl flex items-center justify-center shadow-md" style={{ background: profile.theme?.accentColor || '#818cf8' }}><Sparkles className="w-4 h-4" style={{ color: profile.theme?.backgroundColor || '#0f172a' }} /></div>
                            }
                            <div>
                              <p className="font-bold text-sm text-white">{profile.name || 'بدون اسم'}</p>
                            </div>
                          </div>
                        </div>
